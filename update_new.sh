...
if [ $1 == 'host' ]; then
	...
	if [[ -f /etc/xiaoya/proxy.txt ]] && [[ -s /etc/xiaoya/proxy.txt ]]; then
		...
		docker run -d --env HTTP_PROXY="$proxy_url" --env HTTPS_PROXY="$proxy_url" --env no_proxy="*.aliyundrive.com" --network=host -v /etc/xiaoya:/data --restart=always --name=xiaoya xiaoyaliu/alist:latest
	else	
		docker run -d --network=host -v /etc/xiaoya:/data --restart=always --name=xiaoya xiaoyaliu/alist:latest
	fi	
	exit
fi
fi
